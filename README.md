...
| 命令 | 说明 |
|------|------|
| `mitigate-copy-fail` | 缓解 Copy Fail (禁用 `algif_aead` 模块) |
| `mitigate-dirty-frag` | 缓解 Dirty Frag (禁用 `esp4`/`esp6`/`rxrpc`) |
| `mitigate-all` | 缓解所有漏洞 |
| `rollback-copy-fail` | 回滚 Copy Fail 缓解 |
| `rollback-dirty-frag` | 回滚 Dirty Frag 缓解 |
| `rollback-all` | 回滚所有缓解 |

> **注意**：缓解脚本需要 root 权限运行，执行前会检测 IPsec 使用情况并请求确认。
